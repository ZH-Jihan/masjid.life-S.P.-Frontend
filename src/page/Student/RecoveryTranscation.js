...
const RecoveryTranscation = () => {
  const { data : donars } = useGetData("/transaction/find-donar");
...
const HandleRecoveryPost = async (event) =>{
    event.preventDefault();
    
    const data = {
      donar: selected.selectedId,
      amount: event.target.amount.value,
      donateBy: selected.wayOfTransaction,
      transactionId: event.target.transcationId.value,
    }
    const response = await CustomAxiosPost("/transaction", data)
    if (response?.message === "Success") {
      setselected({});
      toast.success("Successfully Submited Recovery Amount");
      event.target.reset();
    }
      ...
      <div class="flex items-center justify-center mt-4">
        <div class="mx-auto w-full max-w-[700px] border rounded-xl shadow-xl p-8">
          <h1 className="font-bold text-3xl text-center mb-8">
            Submit Recovery Amount
          </h1>
          <form onSubmit={HandleRecoveryPost}>
            <div class="-mx-3 flex flex-wrap">
              <div class="w-full px-3 sm:w-1/3">
                ...
                >
                  ...
                  {donars.map((donar) => (
                    <option value={donar?.donarInfo[0]._id}>{donar.donarInfo[0].fullName}</option>
                  ))}
                </select>
              </div>
...
export default RecoveryTranscation;
